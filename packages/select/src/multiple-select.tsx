...
>(({
  ...
  const valueLabel = useMemo(() => {
    ...
    return value.map((v) => {
      return optionList.find((o) => o.value === v)?.name ?? v
    }).join('、')
  }, [optionList, value])
  ...
  const handleSelect = (optionValue: any) => {
    if(value?.includes(optionValue)) {
      onValueChange?.(value.filter((v) => v !== optionValue) as string[])
    } else {
      onValueChange?.([...value  as string[], optionValue])
    }
  }
              ...
              optionList.map((option, i) => (
                <Ripple
                  ...
                  {...getItemProps({
                    ...
                    className: clsx('cm-select-list-item', value?.includes(option.value) && 'cm-select-list-selected-item', activeIndex === i && 'cm-select-list-actived-item'),
